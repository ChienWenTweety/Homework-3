...
router.post('/transferCoin', function (req, res, next) {
  ...
  let bank = new web3.eth.Contract(contract.abi);
  bank.options.address = req.body.address;
  bank.methods.transferCoin(req.body.value).send({
    //not sure
    to: account
...
router.post('/transferOwner', function (req, res, next) {
  ...
  let bank = new web3.eth.Contract(contract.abi);
  bank.options.address = req.body.address;
  bank.methods.transferOwner(req.body.value).send({
    //要再補上
    from: req.body.account,
    to:
  })
...
router.post('/transferTo', async function (req, res, next) {
  // TODO
  let bank = new web3.eth.sendTransaction(contract.abi);
  bank.options.address = req.body.address;
  bank.methods.transferTo(req.body.value).send({
    to:
    from: req.body.account,
    gas: 3400000
  })
