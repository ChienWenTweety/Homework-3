...
let changeOwnerAddress = $('#changeOwnerAddress');
let changeOwnerAddressButton = $('#changeOwnerAddressButton');

let transferETHAddress = $('#transferETHAddress');
...
update.on('click', function () {
	if (bankAddress != "") {
		$.get('/allBalance', {
			...
			$('#NCCUCoinBalance').text('NCCU COIN 餘額: ' + result.coinBalance)
		})
	}
	else {
		$.get('/balance', {
...
$.get('/Owner', function (get) {
	for (let account of accounts) {
		ownerAddress.append(`<option value="${account}">${account}</option>`)
	}
	nowAccount = whoami.val();
